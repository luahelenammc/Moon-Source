...
The V4 generation makes Codex a first-class surface and routes by object geometry rather than by the presence of a document or the prestige of a model label. Subversion **4.1-public** adds a configurable execution setup: an entitlement or subscription is treated only as one signal, while the effective route is calculated from a reusable **Execution Profile**, volatile **Run State** and **Task Requirements**.
