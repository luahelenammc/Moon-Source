# Moon Source Setup

A beginner-friendly guided portable for creating reusable personal AI context.

[**Download / open Moon Source Setup 2.0 (.md)**](https://raw.githubusercontent.com/luahelenammc/Moon-Source/main/portables/setup/MOON_SOURCE_SETUP.md)

- Current version: 2.0
- Canonical file: [MOON_SOURCE_SETUP.md](MOON_SOURCE_SETUP.md)
- Language: English-first portable with language-sensitive operation
- Dependencies: none
- Claim ceiling: published setup; no impact or adoption claim

Want the whole public architecture instead? [Download the complete Moon Source repository (.zip)](https://github.com/luahelenammc/Moon-Source/archive/refs/heads/main.zip).

Return to the [download hub](../../DOWNLOADS.md) or [portable registry](../../registry/PUBLIC_PORTABLES.md).
